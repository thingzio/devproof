...
```shell
git clone https://github.com/thingzio/devproof && cd devproof
make test         # unit tests
make test-golden  # golden-fixture conformance for the bundle format
make lint         # golangci-lint, actionlint, go vet
...
See [docs/README.md](docs/README.md) for the design index, and
[docs/testing.md](docs/testing.md) for how the test suite is organized
(golden fixtures, fuzzing, conformance).
...
Open an [issue](https://github.com/thingzio/devproof/issues/new/choose) and use
...
#### The bundle format is a compatibility surface

Changes under `internal/canonical/`, `bundle/format.go`, and
`**/testdata/format/` can reissue the identity of every artifact DevProof has
ever produced. `.github/CODEOWNERS` calls those paths out separately for that
reason, and `docs/compatibility.md` states what is guaranteed. Golden fixtures
for a released format version are immutable -- if a change makes
`make test-golden` fail, that is the signal to stop and think, not to run
`make regen-golden`.
...
Open an [issue](https://github.com/thingzio/devproof/issues/new/choose) with the
