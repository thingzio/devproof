...
def sections(page: str):
    """Yield (heading, rows) in document order."""
    heading = "root"
    for part in re.findall(r"(<h[1-6][^>]*>.*?</h[1-6]>|<table.*?</table>)", page, re.S):
...
def entries(page: str):
    ...
    for heading, rows in sections(page):
...
def write_catalog(source: dict, page: str, into: pathlib.Path, retrieved: str) -> int:
    ...
    (into / "SOURCE.yaml").write_text(
        ...
        f"retrieved: {yaml_quote(retrieved)}\n",
        encoding="utf-8",
    )
    return count
...
def main() -> None:
    ...
    if args.check:
        # Only the component data is compared; the retrieval date is expected
        # to differ and says nothing about whether the pages changed.
        diff = subprocess.run(
            ["diff", "-r", "-I", "^  retrieved:", str(CATALOG), str(target)],
            capture_output=True, text=True, check=False,
        )
